...
Image *ConvolutionGPU::apply_convolution_constant_memory(Image *image, Kernel *kernel, type_padding padding) {
  ...
  std::cout << "TOTAL_constant -> " << end_time_data_transfer - start_time_data_transfer - (end_convolution - start_convolution) << " + " << end_convolution - start_convolution << " = "
            << end_time_data_transfer - start_time_data_transfer << std::endl;  cudaFree(d_image);
  cudaFree(d_out_img);
  return out_img;
}
Image *ConvolutionGPU::apply_convolution_global_memory(Image *image, Kernel *kernel, type_padding padding) {
  ...
  std::cout << "TOTAL_global -> " << end_time_data_transfer - start_time_data_transfer - (end_convolution - start_convolution) << " + " << end_convolution - start_convolution << " = "
            << end_time_data_transfer - start_time_data_transfer << std::endl;  delete img_padded;
  cudaFree(d_img);
  cudaFree(d_kernel);
  cudaFree(d_out_img);
  return out_img;
...
Image *ConvolutionGPU::apply_convolution_shared_memory(Image *image, Kernel *kernel, type_padding padding) {
  ...
  start_convolution = omp_get_wtime();
  convolution_shared_memory<<<grid_dim, block_dim, sizeof(unsigned char) * (BLOCK_WIDTH + K_SIZE - 1) * (BLOCK_WIDTH + K_SIZE - 1) + sizeof(float) * K_SIZE * K_SIZE>>>(d_img, d_out_img, d_kernel,
                                                                                                                                                                        WIDTH, HEIGHT, K_SIZE, norm);
  end_convolution = omp_get_wtime();
  ...
  end_time_data_transfer = omp_get_wtime();
  Image *out_img = new Image(h_out_img, image->get_width(), image->get_height());
  std::cout << "TOTAL_shared -> " << end_time_data_transfer - start_time_data_transfer - (end_convolution - start_convolution) << " + " << end_convolution - start_convolution << " = "
