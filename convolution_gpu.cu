...
__global__ void convolution_shared_memory(unsigned char *img, unsigned char *out_img, const int WIDTH, const int HEIGHT, const int K_SIZE, const float norm) {
...
Image *ConvolutionGPU::apply_convolution_shared_memory(Image *image, Kernel *kernel, type_padding padding) {
  ...
  auto start = omp_get_wtime();
  convolution_shared_memory<<<grid_dim, block_dim, sizeof(unsigned char) * (BLOCK_WIDTH + K_SIZE - 1) * (BLOCK_WIDTH + K_SIZE - 1) + sizeof(float) * K_SIZE * K_SIZE>>>(d_img, d_out_img, WIDTH, HEIGHT,
                                                                                                                                                                        K_SIZE, norm);
  auto end = omp_get_wtime();
